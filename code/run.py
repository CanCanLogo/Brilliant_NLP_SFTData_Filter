...
def bert_embedding(texts,batch=100):
    tokenizer = BertTokenizer.from_pretrained(r"D:\new_program\pythonProject\model\bert-base-uncased")
    model = AutoModel.from_pretrained(r"D:\new_program\pythonProject\model\bert-base-uncased").cuda()
...
def main(input_file, output_file, K):
    ...
    json.dump(obj=data_li,fp=open(output_file,"w", encoding='utf-8'),indent=2,ensure_ascii=False)


if __name__ == "__main__":
    input_file = r"D:\new_program\pythonProject\pytorchUse\NLPpro1\NLPNewpro2\quality-evaluation\high_quality_3.json"
    output_file = 'seed-instructions.json'
    K = 10000
